...
use super::{openai::OpenAiProvider, ollama::OllamaProvider, anthropic::AnthropicProvider, grok::GrokProvider, deepseek::DeepSeekProvider, cli_passthrough::CliPassthroughProvider, LlmProvider, ProviderError};
...
impl ProviderRegistry {
    pub fn from_config(cfg: &Config) -> Result<Self, ProviderError> {
        ...
        if let Some(oc) = &cfg.openai {
            if let Some(key) = oc.effective_api_key() {
                let base = oc.base_url.clone().unwrap_or_else(|| "https://api.openai.com/v1".into());
                let model = oc.default_model.clone().unwrap_or_else(|| "gpt-4o-mini".into());
                let p = OpenAiProvider::new(base, key, model);
                map.insert("openai".into(), Box::new(p));
            }
        } else if let Ok(key) = std::env::var("OPENAI_API_KEY") {
            let p = OpenAiProvider::new("https://api.openai.com/v1".into(), key, "gpt-4o-mini".into());
            map.insert("openai".into(), Box::new(p));
        }

        if let Some(oc) = &cfg.ollama {
            let base = oc.effective_base_url();
            let model = oc.default_model.clone().unwrap_or_else(|| "llama3.1".into());
        ...
        if let Some(ac) = &cfg.anthropic {
            if let Some(key) = ac.effective_api_key() {
                let base = ac.base_url.clone().unwrap_or_else(|| "https://api.anthropic.com".into());
                let version = ac.effective_version();
                let model = ac.default_model.clone().unwrap_or_else(|| "claude-3-5-sonnet-latest".into());
                let p = AnthropicProvider::new(base, key, version, model);
                map.insert("anthropic".into(), Box::new(p));
            }
        } else if let Ok(key) = std::env::var("ANTHROPIC_API_KEY") {
            let p = AnthropicProvider::new("https://api.anthropic.com".into(), key, "2023-06-01".into(), "claude-3-5-sonnet-latest".into());
            map.insert("anthropic".into(), Box::new(p));
        }
        ...
        if let Some(gc) = &cfg.grok {
            if let Some(key) = gc.effective_api_key() {
                let base = gc.base_url.clone().unwrap_or_else(|| "https://api.x.ai/v1".into());
                let model = gc.default_model.clone().unwrap_or_else(|| "grok-2-latest".into());
                let p = GrokProvider::new(base, key, model);
                map.insert("grok".into(), Box::new(p));
            }
        } else if let Ok(key) = std::env::var("XAI_API_KEY").or_else(|_| std::env::var("GROK_API_KEY")) {
        ...
        if let Some(dc) = &cfg.deepseek {
            if let Some(key) = dc.effective_api_key() {
                let base = dc.base_url.clone().unwrap_or_else(|| "https://api.deepseek.com".into());
                let model = dc.default_model.clone().unwrap_or_else(|| "deepseek-chat".into());
            ...
            let p = DeepSeekProvider::new("https://api.deepseek.com".into(), key, "deepseek-chat".into());
            map.insert("deepseek".into(), Box::new(p));
        }
            ...
            if c.enabled.unwrap_or(false) {
                let prov = if let Some(cmd) = &c.command {
                    CliPassthroughProvider::custom(
                        ...
                        match c.prompt_mode.as_deref() { Some("raw") => super::cli_passthrough::PromptMode::Raw, _ => super::cli_passthrough::PromptMode::Prefixed },
                ...
                } else { CliPassthroughProvider::claude() };
        ...
        if let Some(c) = &cfg.codex_cli {
            if c.enabled.unwrap_or(false) {
                ...
                } else { CliPassthroughProvider::codex() };
        ...
        if let Some(c) = &cfg.gemini_cli {
            if c.enabled.unwrap_or(false) {
                ...
                } else { CliPassthroughProvider::gemini_with_model(None) };
        ...
        Ok(Self { providers: map, cli_keys: cli })
    }

    pub fn get(&self, key: &str) -> Result<&Box<dyn LlmProvider>, ProviderError> {
        self.providers.get(key).ok_or_else(|| ProviderError::Config(format!("unknown provider: {key}")))
    }
    ...
    pub fn is_cli_key(&self, key: &str) -> bool { self.cli_keys.contains(key) }
}
