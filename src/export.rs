...
fn save_json(path: &str, messages: &[ChatMessage], assistant: &str) -> Result<()> {
    ...
    all.push(ChatMessage { role: "assistant".into(), content: assistant.to_string(), name: None, tool_call_id: None });
...
fn save_html(path: &str, messages: &[ChatMessage], assistant: &str) -> Result<()> {
    let mut out = String::from("<html><head><meta charset=\"utf-8\"><title>rusty-cli export</title></head><body>\n");
    for m in messages {
        out.push_str(&format!("<h3>{}</h3>\n<pre>{}</pre>\n", html_escape::encode_text(&m.role), html_escape::encode_text(&m.content)));
    }
